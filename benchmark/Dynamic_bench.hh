...
enum {
    ...
    opt_gr, opt_node, opt_comm, opt_verb, opt_mix, opt_ordr, opt_splt
};
...
void dynamic_prepopulator<DBParams>::prepopulate_ordered() {
    for (uint64_t key = keymin; key <= keymax; ++key) {
        ...
        if constexpr (DBParams::Split == db_split_type::Static) {
            db.tbl_ordered().nontrans_put(ok, ov, key % 3);
        } else {
...
public:
        ...
        while (true) {
            ...
            switch (t) {
                case txn_type::per_record:
                    if (params.ordered) {
                        runner.run_txn_ordered_per_record(300);
                    } else {
                        runner.run_txn_unordered_per_record(300);
                    }
                    break;
    ...
    static int execute(int argc, const char *const *argv) {
        ...
        */
        ...
        std::cout << "Split type: " << DBParams::Split << std::endl;
