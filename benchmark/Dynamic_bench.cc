...
void print_usage(const char *argv_0) {
    ...
    ss << "Usage of " << std::string(argv_0) << ":" << std::endl
       ...
       << "    Specify the type of splitting to use. Can be one of the following:" << std::endl
    ...
    */
    ...
    case db_params_id::Default:
        ...
        case db_split_type::Static:
            if (enable_commute) {
                ret_code = dynamic_access<db_default_sts_commute_params>::execute(argc, argv);
            } else {
                ret_code = dynamic_access<db_default_sts_params>::execute(argc, argv);
            }
            break;
        case db_split_type::Adaptive:
            if (enable_commute) {
                ret_code = dynamic_access<db_default_ats_commute_params>::execute(argc, argv);
            } else {
                ret_code = dynamic_access<db_default_ats_params>::execute(argc, argv);
            }
            break;
        }
        break;
