...
public:
    ...
    static void htbench_runner_thread(ht_table<DBParams>& table, db_profiler& prof, htbench_runner<DBParams>& runner, double duration, txn_result& results) {
        (void) prof;
        table.table_thread_init();
    ...
    static void htbench_barerunner_thread(ht_table<DBParams>& table, db_profiler& prof, htbench_barerunner<DBParams>& runner, uint64_t txn_count, txn_result& results) {
        (void) prof;
        table.table_thread_init();
    ...
    static void workload_generation(std::vector<htbench_runner<DBParams>>& runners, mode_id mode, uint64_t read_txn_size) {
        std::vector<std::thread> thrs;
        (void) mode;
        for (auto& r : runners) {
            thrs.emplace_back(&htbench_runner<DBParams>::gen_workload, &r, read_txn_size);
        }
        for (auto& t : thrs)
            t.join();
    }

    static void bare_workload_generation(std::vector<htbench_barerunner<DBParams>>& runners, mode_id mode, uint64_t read_txn_size) {
        std::vector<std::thread> thrs;
        (void) mode;
        for (auto& r : runners) {
            thrs.emplace_back(&htbench_barerunner<DBParams>::gen_workload, &r, read_txn_size);
        }
        for (auto& t : thrs)
            t.join();
    }

    static std::pair<double, double> run_benchmark(ht_table<DBParams>& db, db_profiler& prof, std::vector<htbench_runner<DBParams>>& runners, double duration_secs) {
        ...
        for (int i = 0; i < num_runners; ++i) {
            ...
            runner_thrs.emplace_back(htbench_runner_thread, std::ref(db), std::ref(prof),
                                     std::ref(runners[i]), duration, std::ref(results[i]));
        }

        for (auto &t : runner_thrs)
            t.join();
        //htbench_runner_thread(db, prof, runners[0], txn_count, results[0]);
    ...
    static std::pair<double, double> run_bare_benchmark(ht_table<DBParams>& db, db_profiler& prof, std::vector<htbench_barerunner<DBParams>>& runners, uint64_t txn_count) {
        int num_runners = runners.size();
        std::vector<std::thread> runner_thrs;
        std::vector<txn_result> results;
        results.resize(size_t(num_runners));
        ...
        for (int i = 0; i < num_runners; ++i) {
            //fprintf(stdout, "runner %d created\n", i);
            runner_thrs.emplace_back(htbench_barerunner_thread, std::ref(db), std::ref(prof),
    ...
    static int execute(int argc, const char *const *argv) {
        ...
        if (bare) {
            std::vector<htbench_barerunner<DBParams>> runners;
            ...
            std::cout
                << "R/O-throughput: " << ro_tp << " ops/sec/thread; "
                << "R/W-throughput: " << rw_tp << " ops/sec/thread" << std::endl;
        } else {
            std::vector<htbench_runner<DBParams>> runners;
            for (int i = 0; i < num_threads; ++i) {
                uint32_t write_threshold = (i < readers) ? 0 :
                    std::numeric_limits<uint32_t>::max();
                runners.emplace_back(i, table, mode, write_threshold, nontrans);
