...
setup_ycsbz_semopts  # Change this accordingly!
...
run_bench () {
  ...
  printf "# Threads" >> $SECONDARY_OUTFILE
  for label in "${LABELS[@]}"
  do
    for k in $(seq 1 $ITERS)
    do
      printf ",$label$HEADER_LABEL [T$k]" >> $OUTFILE
      printf ",$label$HEADER_LABEL [T$k]" >> $SECONDARY_OUTFILE
    done
  done
  printf "\n" >> $OUTFILE
  printf "\n" >> $SECONDARY_OUTFILE
    ...
    do
      ...
      do
        ...
        do
          ...
          wait $pid
          result=$(cat $TEMPOUT | grep -e '^Collapse 2 throughput:' | grep -oE '[0-9.]{2,}')
          real_time_ms=$(cat $TEMPOUT | grep -e '^Real time:' | grep -oE '[0-9.]+')
          secondary=$(cat $TEMPOUT | grep -e '^Collapse 1 throughput:' | grep -oE '[0-9.]{2,}')
          sleep 2
        ...
        then
          ...
          printf ",$secondary" >> $SECONDARY_OUTFILE
          k=$(($k + 1))
        else
          while [ $k -lt $ITERS ]
          do
            printf ",DNF" >> $OUTFILE
            printf ",DNF" >> $SECONDARY_OUTFILE
            k=$(($k + 1))
          done
        fi
      done
...
run() {
  ...
  do
    ...
    then
      ...
      run_bench $OUTFILE $SECONDARY_OUTFILE $BINARY "$CT_FLAGS" "$HEADER_LABEL" $ITERS $THREADS "${MVCC_LABELS[@]}"
    else
      printf "Running OCC on $BINARY $HEADER_LABEL\n"
      run_bench $OUTFILE $SECONDARY_OUTFILE $BINARY "$CT_FLAGS" "$HEADER_LABEL" $ITERS $THREADS "${OCC_LABELS[@]}"
    fi
    if [ $RFILE != $OUTFILE ]
    then
    ...
    if [ $DFILE != $SECONDARY_OUTFILE ]
    then
      mv $DFILE results/dcopy.txt
      join --header -t , -j 1 results/dcopy.txt $SECONDARY_OUTFILE > $DFILE
      rm results/dcopy.txt $SECONDARY_OUTFILE
    fi
  done
}
...
rm -rf results
mkdir results
RFILE=results/ycsb_z_results.txt
DFILE=results/ycsb_z_secondary_results.txt
