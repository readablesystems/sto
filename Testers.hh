...
public:
  ...
  op_record* doOp(DT* q, int op, int me, std::uniform_int_distribution<long> slotdist, Rand transgen) override {
    ...
    case 0: {
      ...
      TransactionTid::lock(lock);
      std::cout << "[" << me << "] Lookup for key " << key << ". Found value " << val << std::endl;
      TransactionTid::unlock(lock);
    ...
    case 1: {
      ...
      TransactionTid::lock(lock);
      std::cout << "[" << me << "] Updated key " << key << " to value " << val << std::endl;
      TransactionTid::unlock(lock);
    ...
    case 2: {
      ...
      TransactionTid::lock(lock);
      std::cout << "[" << me << "] Inserted key " << key << " with value " << val << std::endl;
      TransactionTid::unlock(lock);
    ...
    case 3: {
      ...
      TransactionTid::lock(lock);
      std::cout << "[" << me << "] Removing key " << key <<  std::endl;
      TransactionTid::unlock(lock);
  ...
  void redoOp(RT* q, op_record *op) override {
    ...
    case 0: {
      ...
      int retval;
      if (q->count(key) == 0) {
        std::cout << "Lookup for key " << key << " failed" << std::endl;
        retval = -1;
      } else {
        retval = q->at(key);
      }

      std::cout << "Doing lookup for key " << key << ". Serial returned: "
                << retval << ". Parallel returned: " << val << std::endl;
      assert(retval == val);
      break;
    }
    case 1: {
  ...
  void check(DT* q, RT* q1) override {
    // Execution not reaching this method yet...
    //std::cout << "Starting check!" << std::endl;
    for(int i = 0; i < MAX_VALUE; i++) {
      TRANSACTION {
        int retval1;
        int retval2;
        if(!q->lookup(std::to_string(i), retval1)) {
          std::cout << "Lookup in q for key " << i << " failed" << std::endl;
          retval1 = -1;
        }
        
        if(q1->count(i) == 0) {
          std::cout << "Lookup in q1 for key " << i << " failed" << std::endl;
          retval2 = -1;
        } else {
          retval2 = q1->at(i);
        }
