...
public:
  ...
  bool transGet(const KT& k, VT& retval) {
    ...
    if (e) {
      ...
      Sto::item(this, pack_bucket(bucket(k))).observe(buck_version.unlocked());
      //if (Opacity)
  ...
  bool trans_write(const KT& k, const VT& v) {
    ...
    if (e) {
      ...
      if (!INSERT) {
        ...
        fence();
        unlock(buck.version);
        Sto::item(this, pack_bucket(bucket(k))).observe(buck_vers);
        //if (Opacity)
        //    check_opacity(buck.version);
        return false;
      }
      ...
      if (bucket_item) {
        bucket_item->update_timestamps(prev_version, new_version);
...
public:
  ...
  bool check(TransItem& item, Transaction& t) override {
    if (is_bucket(item)) {
      ...
      return item.check_timestamps(buck.version, t.timestamp());
  ...
  void install(TransItem& item, Transaction& t) override {
    ...
    if (Opacity && has_insert(item)) {
      ...
      if (buck.version.is_nonopaque()) {
        buck.version.set_timestamps(t.commit_tid());
      }
      unlock(buck.version);
    }
#endif
