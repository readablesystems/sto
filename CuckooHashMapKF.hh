#ifndef _CuckooHashMapKF_HH
#define _CuckooHashMapKF_HH
...
template <typename Key> class Cuckoo_KeyFragment {
    public:
    typedef size_t fragment_type;
    static fragment_type create_fragment(const Key& k) {
        return hashed_key<Key>(k);
    }
    static compare_key_status compare_fragment(fragment_type kf, const Key& k) {
        if (kf == hashed_key<Key>(k)) return check_elem;
        else return false_match;
    }
};
template <> class Cuckoo_KeyFragment<int> {
    public:
    typedef int fragment_type;
    static fragment_type create_fragment(const int& k) {
        return k;
    }
    static compare_key_status compare_fragment(fragment_type kf, const int& k) {
        if (kf == k) return true_match;
        else return false_match;
    }
};

/*! CuckooHashMapKF is the hash table class. This DOES use key fragments, so a bucket looks like:
...
class CuckooHashMapKF: public Shared {
     ...
     * CuckooHashMapKF operations can run simultaneously in different
     * threads, this variable is thread local. Note that this variable
     * can be safely shared between different CuckooHashMapKF
    ...
    // means each template instantiation of a CuckooHashMapKF class
    // gets its own per-thread hazard pointer, then each template
    // instantiation of a CuckooHashMapKF class can get its own
...
private:
    ...
    struct Bucket {
        ...
        */
        typedef typename Cuckoo_KeyFragment<Key>::fragment_type key_frag;
        ...
        bool keys_equal(key_type key, size_t pos) {
            compare_key_status st = Cuckoo_KeyFragment<Key>::compare_fragment(kfs[pos], key);
            switch (st) {
        ...
        bool has_elem(size_t pos) {
            return ie[pos] != NULL;
        }

        void setKV(size_t pos, internal_elem* elem, key_type key) {
            kfs[pos] = Cuckoo_KeyFragment<Key>::create_fragment(key);
            ie[pos] = elem;
        }
...
#endif
        key_frag kfs[SLOT_PER_BUCKET];
        internal_elem* ie[SLOT_PER_BUCKET];
    };
    ...
    static struct cuckoo_stats {
        ...
        cuckoo_stats() : num_times_lock_two(0), num_times_lock_three(0) {};
    } stats;
    ...
    void* pack_bucket(Bucket* b) {
        return (void*) ((uintptr_t)b | bucket_bit);
    }
    Bucket* unpack_bucket(void* b) {
        return (Bucket*) ((uintptr_t)b & ~bucket_bit);
    }
...
public:
     ...
     * exception. */
    explicit CuckooHashMapKF(size_t n = Num_Buckets * SLOT_PER_BUCKET) {
...
private:
    ...
    RETRY:
        ...
        if (st == ok) {
            assert(!ti->buckets_[insert_bucket].has_elem(insert_slot));
    ...
    */
    ...
    bool try_migrate_bucket(TableInfo* ti_old, TableInfo* ti_new, size_t old_bucket) {
        ...
        for (size_t i = 0; i < SLOT_PER_BUCKET; i++) {
            ...
            hv = hashed_key<key_type>(elem->key);
    ...
    */
    ...
    static cuckoo_status cuckoopath_move(TableInfo *ti, CuckooRecord* cuckoo_path,
        ...
        if (depth == 0) {
            ...
            //}
            if (!ti->buckets_[bucket].has_elem(cuckoo_path[0].slot)) {
                return ok;
            } else {
                unlock_two(ti, i1, i2);
        ...
        while (depth > 0) {
            ...
            size_t hv = hashed_key<key_type>( elem2->key );
            size_t first_bucket = index_hash(ti, hv);
    ...
    cuckoo_status try_read_from_bucket(const TableInfo *ti, const key_type& key,
        ...
        for (size_t j = 0; j < SLOT_PER_BUCKET; j++) {
            if (!ti->buckets_[i].has_elem(j)) {
                continue;
            }

            if (ti->buckets_[i].keys_equal(key, j)) {
                e = ti->buckets_[i].get_elem(j);
                return ok;
            }
        }
        return failure_key_not_found;
    ...
    inline void add_to_bucket(TableInfo *ti, const key_type& key, const mapped_type& val,
        ...
        assert(!ti->buckets_[i].has_elem(j));
    ...
    cuckoo_status try_add_to_bucket(TableInfo *ti, const key_type& key, const mapped_type& val,
        ...
        for (size_t k = 0; k < SLOT_PER_BUCKET; k++) {
            if (ti->buckets_[i].has_elem(k)) {
                if (!ti->buckets_[i].keys_equal(key, k)) {
                    continue;
                } else {
...
__thread void** CuckooHashMapKF<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::hazard_pointer_old = nullptr;

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
__thread void** CuckooHashMapKF<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::hazard_pointer_new = nullptr;

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
__thread int CuckooHashMapKF<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::counterid = -1;

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
typename std::allocator<typename CuckooHashMapKF<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::Bucket>
CuckooHashMapKF<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::bucket_allocator;

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
typename CuckooHashMapKF<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::GlobalHazardPointerList
CuckooHashMapKF<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::global_hazard_pointers;

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
const size_t CuckooHashMapKF<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::kNumCores =
    std::thread::hardware_concurrency() == 0 ?
    sysconf(_SC_NPROCESSORS_ONLN) : std::thread::hardware_concurrency();

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
std::atomic<size_t> CuckooHashMapKF<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::numThreads(0);

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
typename CuckooHashMapKF<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::cuckoo_stats 
CuckooHashMapKF<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::stats;
#endif
