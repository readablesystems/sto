#ifndef _CuckooHashMapWK_HH
#define _CuckooHashMapWK_HH
...
class CuckooHashMapWK: public Shared {
     ...
     * CuckooHashMapWK operations can run simultaneously in different
     * threads, this variable is thread local. Note that this variable
     * can be safely shared between different CuckooHashMapWK
    ...
    // means each template instantiation of a CuckooHashMapWK class
    // gets its own per-thread hazard pointer, then each template
    // instantiation of a CuckooHashMapWK class can get its own
...
private:
    ...
    packed_type pack_key(internal_elem* key) {
        packed_type pt;
        pt.key = key;
        pt.bucket_bit = 0;
        return pt; 
    ...
    internal_elem* unpack_key(const TransItem& item) {
...
public:
     ...
     * exception. */
    explicit CuckooHashMapWK(size_t n = Num_Buckets * SLOT_PER_BUCKET) {
...
private:
        ...
        if( check_version( v1_i, v1_f) )
            if (st == ok || !overflow ) {
                ...
                // we found the item!
                    ...
                    assert(e != NULL);
                    auto item = Sto::item(this, pack_key(e));
    ...
    */
    ...
    inline void add_to_bucket(TableInfo *ti, const key_type& key, const mapped_type& val,
        ...
        Sto::item(this, pack_key(elem)).add_write().add_flags(insert_tag);
    ...
    cuckoo_status sto_try_add_with_existing_key(const mapped_type& val,
                                                ...
                                                version_type& ev,
                                                version_to_observe& vto) {
        auto item = Sto::item(this, pack_key(elem_in_table));
        if (item.has_write()) {
            if (has_delete(item)) {
            ...
            // then inserted again? oh well... not sure how we can track "existence"
                ...
                vto.observe_me = true;
                vto.version = ev;
                vto.package = pack_key(elem_in_table);
            }
        }
        return failure_key_duplicated;
    }
    ...
    cuckoo_status try_del_from_bucket(TableInfo *ti, const key_type &key, const size_t i,
        ...
        for (size_t j = 0; j < SLOT_PER_BUCKET; j++) {
            ...
            if (eqfn(elem->key, key)) {
                ...
                if (transactional) {
                    auto item = Sto::item(this, pack_key(elem));
                    if (item.has_write()) {
...
__thread void** CuckooHashMapWK<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::hazard_pointer_old = nullptr;

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
__thread void** CuckooHashMapWK<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::hazard_pointer_new = nullptr;

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
__thread int CuckooHashMapWK<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::counterid = -1;

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
typename std::allocator<typename CuckooHashMapWK<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::Bucket>
CuckooHashMapWK<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::bucket_allocator;

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
typename CuckooHashMapWK<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::GlobalHazardPointerList
CuckooHashMapWK<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::global_hazard_pointers;

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
const size_t CuckooHashMapWK<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::kNumCores =
    std::thread::hardware_concurrency() == 0 ?
    sysconf(_SC_NPROCESSORS_ONLN) : std::thread::hardware_concurrency();

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
std::atomic<size_t> CuckooHashMapWK<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::numThreads(0);

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
typename CuckooHashMapWK<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::cuckoo_stats 
CuckooHashMapWK<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::stats(0,0);

template <class Key, class T, unsigned Num_Buckets, bool Opacity, unsigned SLOT_PER_BUCKET>
typename CuckooHashMapWK<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::key_equal
CuckooHashMapWK<Key, T, Num_Buckets, Opacity, SLOT_PER_BUCKET>::eqfn;
#endif
