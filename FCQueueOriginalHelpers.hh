...
inline int first_lsb_bit_indx(_u32 x) {
	if(0==x) 
		return -1;
	return __builtin_ffs(x)-1;
}

inline int first_lsb_bit_indx64(_u64 x) {
	if(0==x) 
		return -1;
	return __builtin_ffsll(x)-1;
}

inline int first_msb_bit_indx(_u32 x) {
	if(0==x) 
		return -1;
	return  __builtin_clz(x)-1;
}

inline int first_msb_bit_indx64(_u64 x) {
	if(0==x) 
		return -1;
...
namespace CCP {
	...
	public:
		...
		inline static int Max(const int a, const int b) {if(a >= b) return a; else return b;}
		inline static int Min(const int a, const int b) {if(a <= b) return a; else return b;}
	};
	...
	public:
		...
		inline static void* byte_aligned_malloc(const size_t size) {return ALIGNED_MALLOC(size, CACHELINE_SIZE);}
	...
	public:
		...
		inline bool compareAndSet(const int expect, const int update) {
			return ((unsigned)expect == CAS32(&_value, expect, update));
		}  
		inline int getAndIncrement() {
			do {
				const _u32 value = _value;
	...
	private:
		VolatileType<_u32> _value;
	public:
		explicit Atomicbool() {_value=FALSE;}
		explicit Atomicbool(const bool value) {
			if(value)
				_value = TRUE;
			else
				_value = FALSE;
		...
		inline bool compareAndSet(const bool expect, const bool update) {
			...
			return ((unsigned)i_expect == CAS32(&_value, i_expect, i_update));
		}  
	...
	public:
		...
		public:
			...
			inline V* getNotSafe(_u32 volatile * stampHolder) const {
				const _u64 save_value = _value.getNotSafe();
				*stampHolder = ((_u32) (save_value & 0xFFFFFFFFUL));
				return (V*) ((ptr_t)(save_value >> 32));
			}
		...
		public:
			...
			inline V* get(_u32 volatile * stampHolder) const {
				const _u64 save_value( _value.get() );
	...
	public://NOT THREAD SAFE, EACH THREAD SHOULD USE DIFFRENT OBJECT !!
		...
		inline bool nextbool() {return (0 != (nextInt(1024) & 1));}
