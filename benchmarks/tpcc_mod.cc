...
static unsigned g_txn_workload_mix[] = { 45, 43, 4, 4, 4, 0}; // default TPC-C workload mix
//static unsigned g_txn_workload_mix[] = { 50, 0, 0, 0, 0, 50 }; // stable TPC-C workload mix
//static unsigned g_txn_workload_mix[] = { 20, 36, 20, 12, 12, 0 }; // modified TPC-C workload mix
...
public:
  ...
  txn_result txn_fulfill_order();

  static txn_result
  TxnFulfillOrder(bench_worker *w)
  {
    ANON_REGION("TxnFulfillOrder:", &tpcc_txn_cg);
    return static_cast<tpcc_worker *>(w)->txn_fulfill_order();
  }
  ...
  get_workload() const
  {
    ...
    if (g_txn_workload_mix[5])
      w.push_back(workload_desc("FulfillOrder", double(g_txn_workload_mix[5])/100.0, TxnFulfillOrder));
    return w;
  }
	...
	if (enable_par_ckp) {
	  ...
	  if (tree->get_tree_id() == 0) {
        ...
        for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++, cnt++) {
          ...
          v.d_next_o_id = NUM_ORDERS * 3000 + 1;
          //v.d_next_o_id = 3001;
...
protected:
  ...
  load()
  {
	...
	uint64_t ID = 1;
	  ...
	  scoped_str_arena s_arena(arena);
	  for (int num_orders = 0; num_orders < NUM_ORDERS;) {
	    void * const txn = db->new_txn(txn_flags, arena, txn_buf());
	    try {
	      const oorder::key k_oo(w, d, ID);
	      //printf("ID is %lu\n", ID);
		...
		v_oo.o_carrier_id = 0;
	      ...
	      if (c >= 2101) {
		const new_order::key k_no(w, d, ID);
		...
		const order_line::key k_ol(w, d, ID, l);
	  ...
	  c++;
	  //printf("Finished customer %u for district %u for warehouse %u\n", c, d, w);
        }
	//printf("Finished district %u for warehouse %u\n", d, w);
      }
      //printf("Finished warehouse %u\n", w);
    }
...
static event_counter tpcc_evt_fulfill_order_fail_to_find("tpcc_fullfill_order_fail_to_find");

tpcc_worker::txn_result
tpcc_worker::txn_fulfill_order()
{
  unsigned ntries = 3;
start_over:
  uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
  uint districtID = RandomNumber(r, 1, 10);
  uint customerID = GetCustomerId(r);
  ...
  try {
    ...
    //const oorder_c_id_idx::key k_oo_idx_0(warehouse_id, districtID, customerID, 0);
    const oorder_c_id_idx::key k_oo_idx_1(warehouse_id, districtID, customerID, numeric_limits<int32_t>::max());
    static_limit_callback<1> c(s_arena.get(), false);
    tbl_oorder_c_id_idx(warehouse_id)->rscan(txn, Encode(obj_key0, k_oo_idx_1), nullptr, c, s_arena.get());
    const bool found0 = c.size();
    bool found1 = false;
    if (found0) {
      oorder_c_id_idx::key k_oo_idx_temp;
      const oorder_c_id_idx::key *k_oo_idx = Decode(*c.values[0].first, k_oo_idx_temp);
      warehouse_id = k_oo_idx->o_w_id;
      districtID = k_oo_idx->o_d_id;
      customerID = k_oo_idx->o_c_id;

      // look up the order
      const oorder::key k_oo(warehouse_id, districtID, k_oo_idx->o_o_id);
      found1 = (tbl_oorder(warehouse_id)->get(txn, Encode(str(), k_oo), obj_v));
      if (found1) {
        oorder::value v_oo_temp;
        const oorder::value *v_oo = Decode(obj_v, v_oo_temp);
        const size_t nitems = v_oo->o_ol_cnt;
        ALWAYS_ASSERT(nitems);

        // purge this shit
        tbl_oorder_c_id_idx(warehouse_id)->remove(txn, Encode(str(), k_oo_idx_temp));

        const oorder::key k_oo_1(warehouse_id, districtID, k_oo_idx->o_o_id);
        //ALWAYS_ASSERT(tbl_oorder(warehouse_id)->get(txn, Encode(str(), k_oo_1), obj_v));
        tbl_oorder(warehouse_id)->remove(txn, Encode(str(), k_oo_1));

        const new_order::key k_no_1(warehouse_id, districtID, k_oo_idx->o_o_id);
        tbl_new_order(warehouse_id)->remove(txn, Encode(str(), k_no_1));

        for (uint ol_number = 1; ol_number <= nitems; ol_number++) {
          const order_line::key k_ol(warehouse_id, districtID, k_oo_idx->o_o_id, ol_number);
          //ALWAYS_ASSERT(tbl_order_line(warehouse_id)->get(txn, Encode(str(), k_ol), obj_v));
          tbl_order_line(warehouse_id)->remove(txn, Encode(str(), k_ol));
        }
      }
    }

    measure_txn_counters(txn, "txn_fullfill_order");
    if (likely(db->commit_txn(txn))) {
      if ((found0 && found1) || !ntries) {
        if (!ntries) tpcc_evt_fulfill_order_fail_to_find.inc();
...
tpcc_worker::txn_order_status()
{
  ...
  try {
    ...
    //printf("orderline's w, d, o_id are %u, %u, %u\n", warehouse_id, districtID, o_id);
    //printf("orderline is called %lu times\n", c_order_line.n);
