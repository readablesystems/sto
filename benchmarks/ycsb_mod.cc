...
static unsigned g_txn_workload_mix[] = { 30, 70, 0, 0 };
...
public:
  ...
  void set_var(uint64_t low, uint64_t high) {
    low_ = low;
    high_ = high;
    interval_ = high - low;
  }
  ...
  txn_read()
  {
    ...
    try {
      const uint64_t k = r.next() % interval_ + low_;
  ...
  txn_write()
  {
    ...
    scoped_str_arena s_arena(arena);
    try {
      tbl->put(txn, u64_varkey(r.next() % interval_ + low_).str(str()), str().assign(YCSBRecordSize, 'b'));
...
//   static volatile bool first_called = true;
...
// 	std::string("/f0/stephentu/"),
//           std::string("/f1/stephentu/"),
//           std::string("/f2/stephentu/"),
//           std::string("./")});
//     const std::vector<std::vector<unsigned> > assignments_given;
//     txn_logger::Init(logfiles.size(), logfiles, assignments_given, NULL, false, false, false);
// #endif
    ...
    void *txn_buf)
{
  ...
  cerr << "[INFO] Started loading USERTABLE range [kstart="
...
protected:
  ...
  load()
  {
    ...
    for (size_t i = 0; i < nthreads; i++) {
      ...
      const size_t keyend = (i == nthreads - 1) ? nkeys : (i + 1) * nkeysperthd;
      ycsb_load_keyrange(
          keystart,
          keyend,
          i,
...
protected:
  ...
  make_workers()
  {
    ...
    const size_t nkeysperworker = nkeys / nthreads;
    for (size_t i = 0; i < nthreads; i++) {
      int keyend = ((i + 1) == nthreads) ? nkeys : nkeysperworker * (i+1);
      ycsb_worker *w =  new ycsb_worker(blockstart + i, r.next(), db, open_tables,
					&barrier_a, &barrier_b);
      w->set_var(nkeysperworker*i, keyend);
      ret.push_back(w);
    }
