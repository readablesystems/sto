...
volatile bool stats_done = false;
int verbose = 0;
...
void stats_client() {
  ...
  const vector<string> counter_names = split(test_counters, ':');
  for (size_t t = 0; t < counter_names.size(); t++)
    std::cerr << "counter_names: " << counter_names[t] << std::endl;
  ...
  //int write_fd = open("./stats_output", O_CREAT|O_WRONLY|O_TRUNC, 0664);
  ofstream myfile;
  myfile.open(stats_file, ios::out | ios::app);
...
bench_runner::run()
{
  ...
  //sleep(20);
  cerr << "Sleeping done, now ACTUALLY starting benchmark" << endl;
...
#ifdef PERF_TIME
  // here, we create a pipe and forks a new process for the stats client
  // server
  thread th(&stats_client);
#endif
...
#ifdef PERF_TIME
  // needs to stop the stats client process
  stats_done = true;
  fprintf(stderr, "About to join thread\n");
  th.join();
#endif
...
// #ifdef PERF_TIME
//   // print out throughput over time stats
//   for (int j = 0; j < count; j++) {
//     for (size_t i = 0; i < nthreads; i++) {
//       std::cout << (j+1)*100000 << "s: " << "[" << history[i][j] << ", 0] " << i << std::endl;
//     }
//   }
// #endif
